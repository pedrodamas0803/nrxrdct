"""
nrxrdct.laue — Laue diffraction simulation utilities.
"""

from .camera import Camera, CalibrationResult
from .crystal import build_b2, build_bcc, crystal_from_cif, crystals_from_cifs
from .interactive import (
    CalibrationState,
    OrientationState,
    interactive_calibration,
    interactive_orientation,
)
from .fitting import (
    ImageRefinementResult,
    StackImageRefinementResult,
    StrainImageRefinementResult,
    StackStrainImageRefinementResult,
    StackStrainThicknessImageRefinementResult,
    IndexResult,
    MixedFitResult,
    OrientationFitResult,
    SpotMatchClassification,
    StackFitResult,
    StackStrainFitResult,
    StackStrainThicknessFitResult,
    StrainFitResult,
    TwinCandidateResult,
    classify_spot_matches,
    fit_orientation,
    fit_orientation_mixed,
    fit_orientation_stack,
    fit_strain_orientation,
    fit_strain_orientation_stack,
    fit_strain_thickness_orientation_stack,
    index_orientation,
    laue_mixed_residuals,
    laue_residuals,
    laue_stack_residuals,
    laue_strain_residuals,
    laue_strain_stack_residuals,
    laue_strain_thickness_stack_residuals,
    refine_orientation_image,
    refine_orientation_image_stack,
    search_orientation_image,
    search_strain_image,
    search_twin_orientation_image,
    refine_strain_image,
    refine_strain_image_stack,
    refine_strain_thickness_image_stack,
    remove_grain_spots,
    run_orientation_local,
    run_orientation_mixed_local,
    run_strain_local,
    run_strain_mixed_local,
)
from .segmentation import run_segmentation_local
from .laue_plotting import (
    plot_2theta_chi,
    plot_all,
    plot_compare_spots,
    plot_interactive_tth_chi,
    plot_layer_contributions,
    plot_layer_scheme,
    plot_hkl_family_classification,
    plot_laue_comparison,
    plot_laue_stack_spots,
    plot_measured_vs_simulated,
    plot_depth_elongation,
    plot_depth_scan_image,
    plot_pix_deviation,
    plot_multigrain,
    plot_crystals_in_lab,
    plot_detector_projection,
    plot_qspace_around_spot,
    plot_qspace_on_detector,
    plot_qspace_summary,
    plot_rod_qspace_warp,
    plot_rod_tangency,
    plot_rod_tangency_harmonics,
    plot_rod_tangency_blocks,
    plot_simulated_qspace_around_spot,
    plot_simulated_rod_qspace,
    plot_spot_image,
    plot_segmentation,
    plot_strain_broadening,
    plot_tth_chi_overlay,
    plot_twin_search,
    plot_unit_cell,
    plot_unit_cell_in_lab,
    warp_image_to_tth_chi,
    warp_rod_to_qspace,
)
from .layers import (
    Layer,
    LayeredCrystal,
    combine_stacks,
    d_spacing_hkl,
    or_baker_nutting,
    or_from_directions,
    or_kurdjumov_sachs,
    or_nishiyama_wassermann,
    or_pitsch,
    orientation_along_plane,
    orientation_along_z,
    plane_normal_cartesian,
    pseudomorphic_d_spacing,
    pseudomorphic_d_spacing_nonpolar,
    nitride_elastic_constants,
    shortest_lattice_vector,
)
from .segmentation import (
    auto_init_gaussian_mixture_global,
    convert_spotsfile2peaklist,
    convert_spotsfiles_to_dat,
    filter_and_rescale_images,
    fit_gaussian_2d_rotated,
    fit_gaussian_mixture_2d,
    fwhm_from_sigma,
    gaussian_2d_rotated,
    gaussian_mixture_2d,
    get_spot_limits,
    label_segmented_image,
    load_images,
    LoG_segmentation,
    WTH_segmentation,
    hybrid_segmentation,
    measure_peaks,
    plot_labeled_image,
    process_one_image,
    r_squared_image,
    reduced_chi_squared,
    reduced_chi_squared_poisson,
    segment_image,
    simulation_guided_segmentation,
    write_h5_spotsfile,
    write_peaklist_dat,
    fill_gaps_nearest,
    clean_segmentation,
    gaussian_background,
)
from .map import GrainMap, parse_scan_title
from .layered_map import LayeredMap
from .fit_io import read_fit_file, read_raw, prepare_image, F_from_UBB0, plot_fit_frame, inspect_fit_map
from .simulation import (
    BM32_KB,
    beam_divergence_ellipses,
    print_absorption_table,
    clear_allowed_hkl_cache,
    precompute_allowed_hkl,
    beam_in_crystal,
    decompose_matstarlab,
    en2lam,
    estimate_instrument_broadening,
    euler_to_U,
    fit_strain_distribution,
    kb_reflectivity,
    lam2en,
    layer_contributions_spots,
    lorentz_pol,
    measure_spot_widths,
    print_bragg_table,
    print_hkl_family,
    print_layer_contributions,
    print_mixed_summary,
    print_spot_table,
    qspace_around_spot,
    qspace_multi_hkl,
    qspace_per_layer,
    project_to_detector,
    rod_tangency,
    rod_positions,
    rod_satellite_along_positions,
    simulate_qspace_around_spot,
    simulate_rod_qspace,
    simulate_spot_image,
    simulate_full_detector_image,
    submit_full_detector_image,
    collect_full_detector_image,
    simulate_laue,
    simulate_laue_darwin,
    simulate_laue_stack,
    simulate_mixed_phases,
    depth_scan_reconstruction,
    depth_scan_image,
    spectrum_bm,
    spectrum_undulator,
    spot_LP_and_SE,
    strain_broadening,
    strain_spot_jacobian,
    synchrotron_spectrum,
    U_from_matstarlab,
    rotate_U_about_axis,
    rotate_U_about_crystal_axis,
    map_to_fundamental_zone,
    symmetry_equivalent_axes,
    disorientation,
    filter_spots_by_peaks,
)

__all__ = [
    # Beam divergence broadening
    "beam_divergence_ellipses",
    # Energy / wavelength conversion
    "en2lam",
    "lam2en",
    # Synchrotron spectra
    "spectrum_bm",
    "spectrum_undulator",
    "synchrotron_spectrum",
    # KB mirror optics
    "kb_reflectivity",
    "BM32_KB",
    # Crystal builders
    "build_bcc",
    "build_b2",
    "crystal_from_cif",
    "crystals_from_cifs",
    # Orientation
    "euler_to_U",
    "U_from_matstarlab",
    "decompose_matstarlab",
    "beam_in_crystal",
    "rotate_U_about_axis",
    "rotate_U_about_crystal_axis",
    "map_to_fundamental_zone",
    "symmetry_equivalent_axes",
    "disorientation",
    # Strain analysis
    "measure_spot_widths",
    "estimate_instrument_broadening",
    "fit_strain_distribution",
    "strain_spot_jacobian",
    "strain_broadening",
    # Physics helpers
    "lorentz_pol",
    "spot_LP_and_SE",
    # Camera / detector
    "Camera",
    "CalibrationResult",
    # Simulation
    "print_absorption_table",
    "clear_allowed_hkl_cache",
    "precompute_allowed_hkl",
    "simulate_laue",
    "simulate_laue_stack",
    "simulate_laue_darwin",
    "simulate_mixed_phases",
    "depth_scan_reconstruction",
    "depth_scan_image",
    "filter_spots_by_peaks",
    "print_spot_table",
    "print_bragg_table",
    "print_hkl_family",
    "print_layer_contributions",
    "print_mixed_summary",
    "layer_contributions_spots",
    "qspace_around_spot",
    "qspace_multi_hkl",
    "qspace_per_layer",
    "project_to_detector",
    "rod_tangency",
    "rod_positions",
    "rod_satellite_along_positions",
    "simulate_qspace_around_spot",
    "simulate_rod_qspace",
    "simulate_spot_image",
    "simulate_full_detector_image",
    "submit_full_detector_image",
    "collect_full_detector_image",
    # Layered structures
    "Layer",
    "LayeredCrystal",
    "combine_stacks",
    "orientation_along_z",
    "orientation_along_plane",
    "plane_normal_cartesian",
    "shortest_lattice_vector",
    "or_from_directions",
    "or_kurdjumov_sachs",
    "or_nishiyama_wassermann",
    "or_baker_nutting",
    "or_pitsch",
    "d_spacing_hkl",
    "pseudomorphic_d_spacing",
    "pseudomorphic_d_spacing_nonpolar",
    "nitride_elastic_constants",
    # Grain map (raster micro-Laue)
    "GrainMap",
    "LayeredMap",
    "parse_scan_title",
    # Interactive orientation / calibration tools
    "OrientationState",
    "interactive_orientation",
    "CalibrationState",
    "interactive_calibration",
    # Autoindexing
    "IndexResult",
    "index_orientation",
    # Image-based refinement
    "ImageRefinementResult",
    "StackImageRefinementResult",
    "StrainImageRefinementResult",
    "StackStrainImageRefinementResult",
    "StackStrainThicknessImageRefinementResult",
    "refine_orientation_image",
    "refine_orientation_image_stack",
    "search_orientation_image",
    "search_strain_image",
    "search_twin_orientation_image",
    "TwinCandidateResult",
    "SpotMatchClassification",
    "classify_spot_matches",
    "refine_strain_image",
    "refine_strain_image_stack",
    "refine_strain_thickness_image_stack",
    # Orientation fitting
    "OrientationFitResult",
    "StackFitResult",
    "StackStrainFitResult",
    "StackStrainThicknessFitResult",
    "MixedFitResult",
    "StrainFitResult",
    "laue_residuals",
    "laue_stack_residuals",
    "laue_strain_stack_residuals",
    "laue_strain_thickness_stack_residuals",
    "laue_mixed_residuals",
    "laue_strain_residuals",
    "fit_orientation",
    "fit_orientation_stack",
    "fit_orientation_mixed",
    "fit_strain_orientation",
    "fit_strain_orientation_stack",
    "fit_strain_thickness_orientation_stack",
    "remove_grain_spots",
    "run_orientation_local",
    "run_orientation_mixed_local",
    "run_strain_local",
    "run_strain_mixed_local",
    "run_segmentation_local",
    # Plotting
    "plot_2theta_chi",
    "plot_all",
    "plot_compare_spots",
    "plot_interactive_tth_chi",
    "plot_layer_contributions",
    "plot_layer_scheme",
    "plot_hkl_family_classification",
    "plot_laue_comparison",
    "plot_laue_stack_spots",
    "plot_measured_vs_simulated",
    "plot_depth_elongation",
    "plot_depth_scan_image",
    "plot_pix_deviation",
    "plot_multigrain",
    "plot_crystals_in_lab",
    "plot_detector_projection",
    "plot_qspace_around_spot",
    "plot_qspace_on_detector",
    "plot_qspace_summary",
    "plot_rod_qspace_warp",
    "plot_rod_tangency",
    "plot_rod_tangency_harmonics",
    "plot_rod_tangency_blocks",
    "plot_simulated_qspace_around_spot",
    "plot_simulated_rod_qspace",
    "plot_spot_image",
    "plot_segmentation",
    "plot_strain_broadening",
    "plot_tth_chi_overlay",
    "plot_twin_search",
    "plot_unit_cell",
    "plot_unit_cell_in_lab",
    "warp_image_to_tth_chi",
    "warp_rod_to_qspace",
    # Segmentation
    "auto_init_gaussian_mixture_global",
    "convert_spotsfile2peaklist",
    "convert_spotsfiles_to_dat",
    "filter_and_rescale_images",
    "fit_gaussian_2d_rotated",
    "fit_gaussian_mixture_2d",
    "fwhm_from_sigma",
    "gaussian_2d_rotated",
    "gaussian_mixture_2d",
    "get_spot_limits",
    "label_segmented_image",
    "load_images",
    "LoG_segmentation",
    "WTH_segmentation",
    "hybrid_segmentation",
    "measure_peaks",
    "plot_labeled_image",
    "process_one_image",
    "r_squared_image",
    "reduced_chi_squared",
    "reduced_chi_squared_poisson",
    "segment_image",
    "simulation_guided_segmentation",
    "write_h5_spotsfile",
    "write_peaklist_dat",
    "fill_gaps_nearest",
    "clean_segmentation",
    "gaussian_background",
    # LaueTools .fit file I/O
    "read_fit_file",
    "read_raw",
    "prepare_image",
    "F_from_UBB0",
    "plot_fit_frame",
    "inspect_fit_map",
]
