...
------------------------------------------------------
SLURM worker for image-based orientation + per-layer strain refinement.

Invoked by :meth:`LayeredMap.submit_strain_image` via::

    python -m nrxrdct.laue.workers.slurm_layered_img_strain_worker \\
        --meta-json  path/to/img_strain_meta.json \\
        --frame-indices 0,1,2,...
"""
...
def _process_frame(
    ...
    *,
    ...
    from nrxrdct.laue.fitting import refine_strain_image_stack
...
def main() -> None:
    p = argparse.ArgumentParser(description="LayeredMap image-strain worker")
    ...
    fit_strain    = tuple(meta["fit_strain"])
    ...
    print(
        f"Layered img-strain worker — {len(frame_indices)} frames | "
        f"{len(frames)} images | fit_strain={list(fit_strain)}",
        flush=True,
    )
    ...
    _FIT_KEYS = (
        ...
        "max_angle_deg", "max_shift_px", "strain_scale", "correct_depth",
        "method", "options", "structure_model",
    )
    fit_kwargs = {k: meta[k] for k in _FIT_KEYS if k in meta}

    common = dict(
        out_dir=meta["out_dir"], fit_strain=fit_strain,
        fit_kwargs=fit_kwargs, overwrite=meta.get("overwrite", False),
    )
    ...
    print(
        f"Layered img-strain worker done — {n_ok}/{len(frame_indices)} saved  "
        f"({time.time() - t0:.1f}s)",
        flush=True,
    )
    sys.exit(0)
