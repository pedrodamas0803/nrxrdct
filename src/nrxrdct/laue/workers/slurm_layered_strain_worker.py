...
-------------------------------------------------
SLURM worker for orientation + per-layer strain fitting of a LayeredCrystal.

Invoked by :meth:`LayeredMap.submit_strain` via::

    python -m nrxrdct.laue.workers.slurm_layered_strain_worker \\
...
def main() -> None:
    p = argparse.ArgumentParser(description="LayeredMap strain worker")
    ...
    print(
        f"Layered strain worker — {len(frame_indices)} frames | "
    ...
    print(
        f"Layered strain worker done — {n_ok}/{len(frame_indices)} saved  "
        f"({time.time() - t0:.1f}s)",
        flush=True,
    )
    sys.exit(0)
