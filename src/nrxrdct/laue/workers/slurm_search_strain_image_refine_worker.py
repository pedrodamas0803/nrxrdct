...
-------------------------------------------------------------
SLURM worker for the image-based orientation + strain *search* step.

Same grid-search-then-local-polish approach as
:mod:`slurm_search_image_refine_worker` but calls
:func:`~nrxrdct.laue.fitting.search_strain_image` to jointly refine
orientation and all six strain components after the grid search.

Invoked by :meth:`GrainMap.submit_search_strain_image_refine` via::

    python -m nrxrdct.laue.workers.slurm_search_strain_image_refine_worker \\
        --meta-json path/to/search_strain_img_refine_meta.json             \\
        --frame-indices 0,1,2,...
"""
...
from nrxrdct.laue.fitting import search_strain_image
...
def _process_frame(
    ...
    *,
    ...
    h5_dataset: str,
    U_refs: dict,
    search_strain_img_refine_dir: str,
    fit_strain: list,
    kernel_sigma: float,
    bg_sigma: float,
    E_min: float,
    ...
    for gi_str, U_ref_list in U_refs.items():
        ...
        out_path = os.path.join(
            search_strain_img_refine_dir, f"frame_{frame_idx:05d}_g{gi:02d}.npz"
        )
        ...
        try:
            ...
            print(
                f"  ✗  frame {frame_idx} g{gi}: search strain image refine: {exc}",
                flush=True,
            )
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="nrxrdct Laue image-search strain worker (one SLURM job)"
    )
    ...
    print(
        f"Search strain image-refine worker — {len(frame_indices)} frames | "
    ...
    common = dict(
        ...
        U_refs                       = U_refs,
        search_strain_img_refine_dir = meta["search_strain_img_refine_dir"],
        fit_strain                   = meta.get(
    ...
    print(
        f"Search strain image-refine worker done — {n_total} results saved, "
        f"{time.time() - t0:.1f}s",
        flush=True,
    )
    sys.exit(0)
