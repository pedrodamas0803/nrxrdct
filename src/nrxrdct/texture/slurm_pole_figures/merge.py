...
produced by pole_figure_worker.py, for every hkl requested at launch().
...
def merge(
    ...
    *,
    ...
    """
    Assemble the output HDF5 from per-scan tmp files, for every hkl launched.

    Args:
        tmp_dir (Path): Directory containing scan_XXXX_<hkl>.npy and
            scan_XXXX.meta.json files, plus the launch_meta.json sidecar
            written by launch().
    ...
    Returns:
        dict: Maps each hkl label to a dict with keys ``'n_merged'``,
            ``'n_skipped'``, ``'n_missing'``.
    """
    ...
    hkls = launch_meta["hkls"]
    npt_azim = launch_meta["npt_azim"]
    translation_motor = launch_meta["translation_motor"]
    n_total = len(valid_entries)

    # ── Collect available scans (meta present + every hkl's npy present) ──────
    available: dict[int, Path] = {}
    for meta_path in sorted(tmp_dir.glob("scan_????.meta.json")):
        ii = int(meta_path.name.split("_")[1].split(".")[0])
        if all((tmp_dir / f"scan_{ii:04d}_{hkl}.npy").exists() for hkl in hkls):
            available[ii] = meta_path

    print(f"\n{'='*60}")
    print(f"Merging {len(available)}/{n_total} scans → {output_file.name} [hkls: {list(hkls)}]")
    print(f"{'='*60}\n")

    results = {hkl: {"n_merged": 0, "n_skipped": 0, "n_missing": 0} for hkl in hkls}
    ...
    with h5py.File(output_file, "a") as hout:
        ...
        for hkl, tth_range in hkls.items():
            ...
            for ii in tqdm(range(n_total), desc=f"Merging {hkl}"):
                scan_name = f"scan_{ii:04d}"
                group_path = f"{group}/{scan_name}"

                if group_path in hout and not overwrite:
                    results[hkl]["n_skipped"] += 1
                    continue

                if ii not in available:
                    results[hkl]["n_missing"] += 1
                    continue

                npy_path = tmp_dir / f"{scan_name}_{hkl}.npy"
                meta_path = available[ii]
                ...
                try:
                    ...
                    print(f"  ✗ {scan_name} [{hkl}]: failed to load tmp files — {e}")
                    results[hkl]["n_missing"] += 1
                    continue
                ...
                for k, v in meta.items():
                    if k == "hkls":
                        continue  # per-scan list of all extracted hkls, not a per-hkl attr
                    ds.attrs[k] = v

                results[hkl]["n_merged"] += 1

    print(f"\n{'='*60}")
    for hkl, r in results.items():
        print(f"  {hkl}: merged={r['n_merged']}  skipped={r['n_skipped']}  missing={r['n_missing']}")
    print(f"{'='*60}\n")

    n_missing_total = sum(r["n_missing"] for r in results.values())
    if n_missing_total:
        print(f"  Missing scan indices: {sorted(set(range(n_total)) - set(available))}")
